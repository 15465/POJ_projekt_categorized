...
public class Strategy_random implements Strategy {
    public int count_pawns(Board playground, String color){ // Policzenie zywych pionkow
        ...
        for (int x = 0; x < 7; x++){
            for (int y = 0; y < 7; y++){
                if (playground.board[x][y].occupied && playground.board[x][y].occupant.color.equals("color") && playground.board[x][y].occupied && playground.board[x][y].occupant.moveability[3]){
                    live_pawns++;
                }
            }
        }
    ...
    public void choose_move(Board playground, String color){ // Losowe wybranie ruchu dla wybranego pionka

        Coordinates chosen = choose_pawn(playground, color);
